...
filter_links = ['http://uji.neimanmarcus.com.cn/women/categories/topbottoms/tops.html'];
...
function sortQueryPara(url){
	var vars = url.split('?');
	var queryStr = vars[1].split('&').sort(function(a,b){ 
			var _a = a.split('=')[0]; _b = b.split('=')[0];
			return _a < _b ? -1:1});
	return vars[0] + '?' + queryStr.join('&');
}
		...
		var url_item = new Object(); 
			...
			if (page_info.length > 1) {
				if (current_url.indexOf('?') != -1){
                     page_info[0] = page_info[0].replace('?','&');
  		          }
  		        var temp_openingurl = sortQueryPara(current_url + page_info[0]);  
				if (fileinfo != '') {
				...
				if (same == false) {
					...
					url_item.openingurl = sortQueryPara(current_url + page_info[0]);
				...
				links = this.evaluate(function(){
					...
					for (i = 0; i < design_option.length; i++) {
						if (design_option[i].getAttribute('value')!=0) {
							url_group = url_group + '?designer='+ design_option[i].getAttribute('value') +'|'+ page_title +'- Designer/ '+ design_option[i].innerHTML +';';	
					...
					sizes = this.evaluate(function(){
						...
						for (i = 0; i < size_option.length; i++) {
							if (size_option[i].getAttribute('value')!=0) {
								url_size = url_size + '?size='+size_option[i].getAttribute('value')+'&type=all|'+ page_title +'- type/ all'+ size_option[i].innerHTML +';';
						...
						if (isNaN(maps.length)) {
							jQuery.each(maps,function(i,n){
									jQuery.each(n,function(j,m){
										...
										url_group = url_group + '?size='+ j +'&type='+ i +'|'+ page_title +'- type/'+ type_name +' - size/'+ m +';';															
										//console.log(url_group);
									});
							});
						}
...
//save links to json file
	var date = new Date();
    fs.write('/nm-links/reports/filterLinks-sub-' + date.getTime() + '.txt', results.join('\r\n'));
